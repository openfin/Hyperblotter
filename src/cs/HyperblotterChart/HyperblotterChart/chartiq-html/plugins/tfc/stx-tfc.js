 ...
 * @example
	this.openOrders={
		"IBM":[{id:"1", action:"sell", quantity:500, limit:197, tif:"GTC", currency:"USD"}, // There are four open orders to sell IBM, one is a bracket order on existing position
				{id:"2", action:"sell", quantity:500, limit:196, tif:"GTC", currency:"USD"},
				{id:"9", tradeid:"IBM001", action:"sell", quantity:300, limit:165, tif:"GTC", currency:"USD", oco:"10"},
				{id:"10", tradeid:"IBM001", action:"sell", quantity:300, stop:135, tif:"GTC", currency:"USD", oco:"9"}],

		"TSLA":[{id:"3", action:"buy", quantity:10, limit:170, tif:"DAY", currency:"USD"}],	// There is a single day order to buy TSLA

		"GE":[{id:"4", tradeid:"GE001", action:"sell", quantity:100, limit:30, tif:"GTC", currency:"USD", oco:"5"},
				{id:"5", tradeid:"GE001", action:"sell", quantity:100, stop:25, tif:"GTC", currency:"USD", oco:"4"}
			],

		"LNKD":[{id:"6", action:"buy", quantity:100, limit:112, tif:"DAY", currency:"USD", oto: [	// LNKD has an open order and an OTO bracket (and OTO that contains an OCO)
				{id:"7", action:"sell", quantity:100, limit:130, tif:"GTC", currency:"USD", oco:"8"},
				{id:"8", action:"sell", quantity:100, stop:110, tif:"GTC", currency:"USD", oco:"7"}
				]}
			]
...
STX.Account.prototype.isForex=function(symbol){
	return STX.LegacyMarket.isForexFuturesSymbol(symbol);
};
/**
...
STX.Account.Demo=function(){
	...
	// ************ REMOVE this positions and openOrders data FROM THE CONSTRUCTOR IN A LIVE SYSTEM ******
...
STX.Account.Demo.prototype.placeOrder=function(tfc, order, cb){
	if(order.constructor == Array){
		...
		if(!order.limit && !order.stop){	// market orders
			this.execute(order, tfc.getCurrentPriceForOrder(tfc.activeTrade));
		}else{
...
STX.TFC.prototype.populateMarket=function(){
	...
	if(this.account.positions[this.chart.symbol]){
...
STX.TFC.prototype.getCurrentPriceForOrder=function(activeTrade){
	...
	if ( this.stx.chart.yAxis.minimumPriceTick ) {
		var numToRoundTo = 1 / (this.stx.chart.yAxis.minimumPriceTick);
...
STX.TFC.prototype.enableBracket=function(params){
	...
	if(position.quantity>0){
		...
		STX.swapClassName(this.dom.dragLineBelow, "red", "green");
		STX.appendClassName(this.dom.limitOrder, "new-sell-order");
		this.elements.otoAboveLegLabel.innerHTML=this.stx.translateIf("Take Profit");
		this.elements.otoBelowLegLabel.innerHTML=this.stx.translateIf("Stop Loss");
	}else{
		this.activeTrade="bracket_cover";
		STX.swapClassName(this.dom.dragLineAbove, "green", "red");
		STX.swapClassName(this.dom.dragLineBelow, "green", "red");		
		STX.appendClassName(this.dom.limitOrder, "new-cover-order");
		this.elements.otoBelowLegLabel.innerHTML=this.stx.translateIf("Take Profit");
		this.elements.otoAboveLegLabel.innerHTML=this.stx.translateIf("Stop Loss");
	}
...
STX.TFC.prototype.priceFromLocation=function(y){
	...
	// snap the price to the desired interval if one defined
	if ( this.stx.chart.yAxis.minimumPriceTick ) {
		var numToRoundTo = 1 / (this.stx.chart.yAxis.minimumPriceTick);
		price = Math.round(price * numToRoundTo) / numToRoundTo;
	}
	...
	function update(){
		...
		for(j=0;j<symbolList.length;j++){
			...
			for(i=0;i<openOrders.length;i++){
				...
				price=0;
				if(openOrder.limit){
					STX.newChild(tr, "TD", null, openOrder.oco?(openOrder.quantity<0?"@SL":"@TP"):"@LMT");
					STX.newChild(tr, "TD", null, openOrder.limit);
					price=openOrder.limit;
				}else if(openOrder.stop){
					STX.newChild(tr, "TD", null, openOrder.oco?(openOrder.quantity<0?"@TP":"@SL"):"@STP");
		...
		if(!foundOne){
			STX.newChild(STX.newChild(table, "TR"), "TD", "tfc-not-found", self.stx.translateIf("No Open Orders"));
		}
		...
		for(j=0;j<symbolList.length;j++){
			...
			if(viewType=="lots"){
				...
				td=STX.newChild(tr, "TD", null, symbol+" @"+position.price);
				td.setAttribute("colspan",5);
				...
				STX.newChild(tr, "TD", "tfc-col-qty", position.quantity);
				STX.newChild(tr, "TD", null, position.basis);
				td=STX.newChild(tr, "TD", "stx-btn click", self.stx.translateIf("Close Position"));
				if(position.quantity>0){
			...
			if(viewType=="lots" || viewType=="maintenance"){
				if(self.account.trades[symbol]){
					for(var t=0;t<self.account.trades[symbol].length;t++){
						...
						td=STX.newChild(tr, "TD", "tfc-col-qty", trade.quantity);
						td=STX.newChild(tr, "TD", null, trade.basis);
						if(viewType=="lots"){
						...
						if(viewType=="maintenance"){
							...
							td=STX.newChild(tr, "TD", "stx-btn click", self.stx.translateIf("Close"));
							if(trade.quantity>0){
		...
		if(!foundOne){
			tr=STX.newChild(table, "TR");
			td=STX.newChild(tr, "TD", "tfc-not-found", self.stx.translateIf("No Positions"));
			//td.setAttribute("colspan", 5);
...
STX.TFC.prototype.cancelOpenOrder=function(){
	...
	this.account.confirmOrder(this, order, function(confirmation){
		...
		if(order.tif=="DAY") tifNode.innerHTML=self.stx.translateIf("Day Order");
		else if(order.tif=="GTC") tifNode.innerHTML=self.stx.translateIf("Good Until Cancelled");
...
STX.TFC.prototype.createOCOFromGUI=function(){
	...
	var arr=[];
	if(this.dom.otoAbove.style.display!="none") arr.push(order);
	if(this.dom.otoBelow.style.display!="none") arr.push(order2);
	return arr;
};
...
STX.TFC.prototype.confirmOCO=function(order, type){
	...
	for(i=0;i<order.length;i++){
		...
		if(type=="bracket"){
			if(oco.stop) orderLabels[i].innerHTML=this.stx.translateIf("Stop Loss")+":";
			else if(oco.limit) orderLabels[i].innerHTML=this.stx.translateIf("Take Profit")+":";
			headerNode.innerHTML=this.stx.translateIf("Confirm: Bracket / Protect");
		}else{
			orderLabels[i].innerHTML=this.stx.translateIf("Order")+" "+(i+1)+":";
			headerNode.innerHTML=this.stx.translateIf("Confirm: One Cancels the Other");
		}
	}

	if(order[0].tif=="DAY") tifNode.innerHTML=this.stx.translateIf("Day Order");
	else if(order[0].tif=="GTC") tifNode.innerHTML=this.stx.translateIf("Good Until Cancelled");
...
STX.TFC.prototype.construct=function(config){
	...
	this.dom.dragLineAbove=STX.cloneUIComponent($$$(".drag-price-line", container));
	this.dom.dragLineCenter=STX.cloneUIComponent($$$(".drag-price-line", container));
	this.dom.dragLineBelow=STX.cloneUIComponent($$$(".drag-price-line", container));
	this.dom.marketOrder=STX.cloneUIComponent($$$(".market-order", container));
	this.dom.limitOrder=STX.cloneUIComponent($$$(".stx-limit-order", container));
	this.dom.otoAbove=STX.cloneUIComponent($$$(".OTO.stx-stop", container));
	this.dom.otoBelow=STX.cloneUIComponent($$$(".OTO.stx-stop", container));
	this.dom.ocoOrder=STX.cloneUIComponent($$$(".stx-oco-order", container));
	this.dom.ocoAbove=STX.cloneUIComponent($$$(".oco.tfc-oco-above", container));
	this.dom.ocoBelow=STX.cloneUIComponent($$$(".oco.tfc-oco-below", container));
	this.dom.shadeAbove=STX.cloneUIComponent($$$(".tfc-shade", container));
	this.dom.shadeBelow=STX.cloneUIComponent($$$(".tfc-shade", container));
