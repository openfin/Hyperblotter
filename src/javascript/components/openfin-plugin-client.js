...
var errorTopic = '__blpapi-error';
var requestTopic = '__blpapi-request';
...
function Session() {
    ...
    this.socket.on(errorTopic, function (m) {
        // receive blpapi exceptions from the server and show them in console
...
Session.prototype.on = function (event, listener) {
    ...
    this.socket.on(event, function (m) {
    ...
    this.socket.emit(requestTopic, { functionName: 'on', listenerId: listenerId, event: event });
};
...
function bindInvokeFunction(functionName) {
    return function () {
        ...
        this.socket.emit(requestTopic, { functionName: functionName, args: [].concat(Array.prototype.slice.call(arguments)) });
    };
}

// todo: temporary namespace for OpenFin plugin architecture
fin.desktop.Plugins = {};
fin.desktop.Plugins.blpapi = { Session: Session };
